...
@skip_if_no_tools
def test_character_value_attribute():
    """Test Character'Value attribute for string to character conversion."""
    source = """
    with Ada.Integer_Text_IO;
    with Ada.Text_IO;
    procedure Test is
    ...
    begin
        ...
        C := Character'Value("'z'");  -- lowercase
        Ada.Integer_Text_IO.Put(Character'Pos(C));
        Ada.Text_IO.New_Line;

        C := Character'Value("   'X'");  -- with leading whitespace
        Ada.Integer_Text_IO.Put(Character'Pos(C));
        Ada.Text_IO.New_Line;
    end Test;
    """
...
@skip_if_no_tools
def test_wide_character_value_attribute():
    """Test Wide_Character'Value attribute for string to wide character conversion."""
    # Note: Uses Integer to hold result to avoid Wide_Character constraint check issue
    source = """
    with Ada.Integer_Text_IO;
    with Ada.Text_IO;
    procedure Test is
        Val : Integer;
    begin
        Val := Wide_Character'Pos(Wide_Character'Value("'A'"));
        ...
        Val := Wide_Character'Pos(Wide_Character'Value("'z'"));  -- lowercase
        Ada.Integer_Text_IO.Put(Val);
        Ada.Text_IO.New_Line;

        Val := Wide_Character'Pos(Wide_Character'Value("   'X'"));  -- with leading whitespace
        Ada.Integer_Text_IO.Put(Val);
        Ada.Text_IO.New_Line;
    end Test;
    """
...
@skip_if_no_tools
def test_wide_character_image_attribute():
    """Test Wide_Character'Image attribute for wide character to string conversion."""
    # Use Integer for the code point and call _wc_img directly via Image
    # This avoids Wide_Character'Val constraint check issues
    source = """
    with Ada.Text_IO;
    with Ada.Integer_Text_IO;
    procedure Test is
        Code : Integer;
    begin
        -- Test by outputting the character code and using Wide_Character'Image
        -- via type conversion workaround
        Code := 65;  -- 'A'
        Ada.Text_IO.Put_Line(Wide_Character'Image(Wide_Character'Val(Code)));
    ...
    lines = stdout.strip().split('\n')
    assert len(lines) >= 3, f"Expected 3 lines, got: {stdout}"
    # Wide_Character'Image returns "'X'" format
    assert "'A'" in lines[0], f"Expected 'A' in output, got: {lines[0]}"
    assert "'z'" in lines[1], f"Expected 'z' in output, got: {lines[1]}"
    assert "'5'" in lines[2], f"Expected '5' in output, got: {lines[2]}"
...
@skip_if_no_tools
def test_float_image_attribute():
