"""
Z80 Post-assembly optimization pass.

This module provides language-agnostic post-assembly optimizations for Z80.
Originally extracted from uplm80, adapted for reuse across multiple compilers.
...
def parse_procedures(lines: list[str],
                     proc_label_pattern: str | None = None) -> list[Procedure]:
    """
    Extract procedures from assembly lines.

    Args:
        lines: Assembly source lines
        proc_label_pattern: Optional regex pattern to match procedure labels.
                           Default matches labels containing '@' or starting uppercase.
    """
    procs = []

    # Default pattern: matches PL/M-style and Ada-style procedure labels
    if proc_label_pattern is None:
        def is_proc_label(label: str) -> bool:
            return '@' in label or label[0].isupper()
    else:
        pattern = re.compile(proc_label_pattern)
        def is_proc_label(label: str) -> bool:
...
def optimize_asm(asm_code: str, verbose: bool = False,
                 proc_label_pattern: str | None = None) -> tuple[str, int]:
    """
    ...
    Args:
        asm_code: Assembly source code as string
        verbose: Print optimization details
        proc_label_pattern: Optional regex for procedure label detection
    ...
    while pass_num < max_passes:
        ...
        procs = parse_procedures(lines, proc_label_pattern)
...
def optimize(input_path: str, output_path: str | None = None,
             proc_label_pattern: str | None = None) -> int:
    """
    Run post-assembly optimizations on a .mac file.

    Args:
        input_path: Input assembly file path
        output_path: Output file path (defaults to input_path)
        proc_label_pattern: Optional regex for procedure label detection
    ...
    optimized, savings = optimize_asm(asm_code, verbose=True,
                                       proc_label_pattern=proc_label_pattern)
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 2:
        print("Usage: python -m shared.z80_postopt input.mac [output.mac]")
        sys.exit(1)
