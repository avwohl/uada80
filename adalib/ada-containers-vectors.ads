-- Ada.Containers.Vectors for Z80
-- Generic unbounded vector (growable array)
--
-- Note: On Z80 with limited memory, vectors have practical limits.
-- This is a simplified implementation using static maximum capacity.
...
private
   ...
   -- Maximum vector size for Z80 (can be adjusted)
   ...
   type Vector is tagged record
      Elements : Element_Array (Index_Type'First .. Index_Type'First + Index_Type'Base (Max_Capacity) - 1);
   ...
   Empty_Vector : constant Vector := (Elements => (others => <>), Last => No_Index);
