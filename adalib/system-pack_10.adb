...
package body System.Pack_10 is
   ...
   is
      Bit_Offset : constant Natural := N * 10;
   ...
   begin
      ...
      if Bit_Pos > 6 then
         ...
         begin
            Val := Val + Natural (B3 mod (2 ** (Bit_Pos + 10 - 16))) * 256;
         end;
      end if;

      return Bits_10 (Val mod 1024);
   end Get_10;
   ...
   begin
      ...
      if Bit_Pos <= 6 then
         B2 := (B2 and not Cluster (2 ** (Bit_Pos + 2) - 1)) or
               Cluster ((V / (2 ** (8 - Bit_Pos))) mod 256);
      else
      ...
      if Bit_Pos > 6 then
         ...
         begin
            B3 := (B3 and not Cluster (2 ** (Bit_Pos + 10 - 16) - 1)) or
                  Cluster (V / 256);
