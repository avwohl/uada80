-- Ada.Containers.Bounded_Hashed_Maps body for Z80
-- Generic bounded hashed map implementation
...
package body Ada.Containers.Bounded_Hashed_Maps is
   ...
   function Bucket_For (Container : Map; Key : Key_Type) return Ada.Containers.Hash_Type is
   begin
      return Hash (Key) mod Container.Modulus;
   end Bucket_For;
   ...
   begin
      ...
      for I in Left.Nodes'Range loop
         if Left.Nodes (I).In_Use then
            Pos := Find (Right, Left.Nodes (I).Key);
            if Pos = No_Element then
               return False;
            end if;
            if Left.Nodes (I).Element /= Element (Pos) then
               return False;
            end if;
         end if;
      end loop;
   ...
   begin
      ...
      Process (Position.Container.Nodes (Position.Node).Key,
               Position.Container.Nodes (Position.Node).Element);
   end Query_Element;
   ...
   begin
      ...
      Process (Container.Nodes (Position.Node).Key,
   ...
   function Find (Container : Map; Key : Key_Type) return Cursor is
      Bucket : constant Ada.Containers.Hash_Type := Bucket_For (Container, Key);
      Node   : Node_Index := Container.Buckets (Bucket);
   begin
      while Node /= No_Node loop
         if Equivalent_Keys (Container.Nodes (Node).Key, Key) then
   ...
   begin
      ...
      Container.Nodes (New_Node).Key := Key;
   ...
   begin
      ...
      Delete (Container, Container.Nodes (Position.Node).Key);
      Position := No_Element;
   end Delete;
