...
package body System.Pack_15 is
   ...
   is
      Bit_Offset : constant Natural := N * 15;
   ...
   begin
      ...
      if Bit_Pos > 1 then
         ...
         begin
            Val := Val + Natural (B3 mod (2 ** (Bit_Pos + 15 - 16))) * 256;
         end;
      end if;

      return Bits_15 (Val mod 32768);
   end Get_15;
   ...
   begin
      ...
      if Bit_Pos > 1 then
         ...
         begin
            B3 := (B3 and not Cluster (2 ** (Bit_Pos + 15 - 16) - 1)) or
                  Cluster (V / 256);
