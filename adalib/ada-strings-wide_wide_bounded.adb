-- Ada.Strings.Wide_Wide_Bounded body for Z80
-- Bounded-length wide wide string handling implementation
...
package body Ada.Strings.Wide_Wide_Bounded is
   ...
   function Length (Source : Bounded_Wide_Wide_String) return Length_Range is
   begin
   ...
   is
      Result : Bounded_Wide_Wide_String;
   begin
      Set_Bounded_Wide_Wide_String (Result, Source, Drop);
      return Result;
   ...
   function To_Wide_Wide_String (Source : Bounded_Wide_Wide_String) return Wide_Wide_String is
   begin
   ...
   end Set_Bounded_Wide_Wide_String;
   ...
   is
   begin
      Append (Source, To_Wide_Wide_String (New_Item), Drop);
   end Append;
   ...
   begin
      if New_Length <= Max_Length then
         ...
         case Drop is
            when Left =>
               declare
                  Combined : Wide_Wide_String (1 .. New_Length);
               begin
   ...
   function "&"
     (Left, Right : Bounded_Wide_Wide_String) return Bounded_Wide_Wide_String
   is
      Result : Bounded_Wide_Wide_String := Left;
   begin
   ...
   is
      Result : Bounded_Wide_Wide_String := To_Bounded_Wide_Wide_String (Left);
   begin
      Append (Result, Right, Error);
      return Result;
   end "&";
   ...
   function "&"
     (Left : Bounded_Wide_Wide_String; Right : Wide_Wide_Character) return Bounded_Wide_Wide_String
   is
      Result : Bounded_Wide_Wide_String := Left;
   begin
      Append (Result, Right, Error);
   ...
   is
      Result : Bounded_Wide_Wide_String := To_Bounded_Wide_Wide_String ((1 => Left));
   begin
      Append (Result, Right, Error);
      return Result;
   end "&";
   ...
   is
   begin
      return To_Bounded_Wide_Wide_String (Slice (Source, Low, High));
   end Bounded_Slice;
   ...
   function "=" (Left, Right : Bounded_Wide_Wide_String) return Boolean is
   begin
      return To_Wide_Wide_String (Left) = To_Wide_Wide_String (Right);
   end "=";

   function "=" (Left : Bounded_Wide_Wide_String; Right : Wide_Wide_String) return Boolean is
   begin
      return To_Wide_Wide_String (Left) = Right;
   end "=";

   function "=" (Left : Wide_Wide_String; Right : Bounded_Wide_Wide_String) return Boolean is
   begin
   ...
   is
      Src : constant Wide_Wide_String := To_Wide_Wide_String (Source);
   begin
   ...
   begin
      ...
      if High < Low then
         return To_Bounded_Wide_Wide_String (Src (1 .. Low - 1) & By & Src (Low .. Source.Length), Drop);
      else
         return To_Bounded_Wide_Wide_String (Src (1 .. Low - 1) & By & Src (High + 1 .. Source.Length), Drop);
      end if;
   end Replace_Slice;
   ...
   begin
      ...
      return To_Bounded_Wide_Wide_String (Src (1 .. From - 1) & Src (Through + 1 .. Source.Length));
   end Delete;
   ...
   begin
      ...
      if Side = Left or Side = Both then
         while First <= Last and then Src (First) = Wide_Wide_Space loop
            First := First + 1;
         end loop;
      end if;

      if Side = Right or Side = Both then
         while Last >= First and then Src (Last) = Wide_Wide_Space loop
            Last := Last - 1;
         end loop;
      end if;

      if First > Last then
         return Null_Bounded_Wide_Wide_String;
      end if;

      return To_Bounded_Wide_Wide_String (Src (First .. Last));
   end Trim;
