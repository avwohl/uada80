-- Ada.Containers.Indefinite_Doubly_Linked_Lists body for Z80
-- Generic doubly linked list for indefinite types implementation
...
package body Ada.Containers.Indefinite_Doubly_Linked_Lists is
   ...
   begin
      ...
      while L_Node /= No_Node loop
         if Left.Nodes (L_Node).Element.all /= Right.Nodes (R_Node).Element.all then
            return False;
         end if;
   ...
   begin
      while Node /= No_Node loop
         if Container.Nodes (Node).Element.all = Item then
