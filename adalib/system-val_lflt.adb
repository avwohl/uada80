-- System.Val_LFlt body for Z80
-- Long_Float value conversion

package body System.Val_LFlt is
      ...
      while Ptr <= Max loop
         if Str (Ptr) = '.' then
            ...
            if Got_Dot then
               ...
               Result := Result + Long_Float (Digit) / Frac_Div;
            else
               Result := Result * 10.0 + Long_Float (Digit);
            end if;
            Ptr := Ptr + 1;
   ...
   function Value_Long_Float (Str : String) return Long_Float is
      Ptr : Integer := Str'First;
      Res : Long_Float;
   begin
      Scan_Long_Float (Str, Ptr, Str'Last, Res);
