-- GNAT.Min_Heap body for Z80
-- Binary min-heap implementation

package body GNAT.Min_Heap is
   ...
   begin
      loop
         ...
         if Left <= H.Count and then H.Data (Left) < H.Data (Smallest) then
            Smallest := Left;
         end if;

         if Right <= H.Count and then H.Data (Right) < H.Data (Smallest) then
            Smallest := Right;
         end if;
   ...
   procedure Heap_Sort (Values : in Out Integer_Array) is
      H : Heap;
   begin
